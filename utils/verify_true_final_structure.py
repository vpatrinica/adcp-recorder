...
def verify_paths():
    ...
    assert not any(f.suffix == ".nmea" for f in binary_files), (
        "Found .nmea file in errors/binary, expected ONLY .dat blobs"
    )
    ...
    assert any("PNORI_error_" in f.name for f in nmea_files), (
        "Missing PNORI_error_*.nmea in errors/nmea/"
    )
    assert any(f.name.startswith("ERROR_") for f in nmea_files), (
        "Missing consolidated ERROR_*.nmea in errors/nmea/"
    )
