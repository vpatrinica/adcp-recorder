...
def search_sql(con, query_str):
    ...
    constraints = con.execute("SELECT table_name, constraint_text FROM duckdb_constraints()").fetchall()
...
def find_dependencies(con, table_name):
    ...
    # Get table OID
    row = con.execute(f"SELECT table_oid FROM duckdb_tables() WHERE table_name = '{table_name}'").fetchone()
    if not row:
    ...
    for oid, cid, d_type in deps:
        ...
        if cid == 0: # Table or View
            r = con.execute(f"SELECT table_name FROM duckdb_tables() WHERE table_oid = {oid} UNION SELECT view_name FROM duckdb_views() WHERE view_oid = {oid}").fetchone()
            if r: name = r[0]
        elif cid == 1: # Index
            r = con.execute(f"SELECT index_name FROM duckdb_indexes() WHERE index_oid = {oid}").fetchone()
            if r: name = r[0]
        elif cid == 2: # Sequence
            r = con.execute(f"SELECT sequence_name FROM duckdb_sequences() WHERE sequence_oid = {oid}").fetchone()
            if r: name = f"Sequence: {r[0]}"
