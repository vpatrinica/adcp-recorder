...
def test_generate_service_linux(runner, tmp_path):
    with patch('adcp_recorder.templates.__file__', '/fake/path/adcp_recorder/templates/__init__.py'):
...
def test_generate_service_windows(runner, tmp_path):
    result = runner.invoke(cli, ['generate-service', '--platform', 'windows', '--out', str(tmp_path)])
