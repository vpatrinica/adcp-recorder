...
class TestPNORWDParser:
    ...
    def test_parse_main_direction(self):
        ...
        sentence = (
            ...
            "326.5,335.8,11.6,8.2,147.6,107.1,74.8,55.4,55.0,50.8*3C"
        )
    ...
    def test_parse_directional_spread(self):
        ...
        sentence = "$PNORWD,DS,120720,093150,1,0.02,0.01,5,45.2,38.7,52.1,41.3,39.8*1B"
    ...
    def test_both_direction_types(self):
        ...
        for dir_type in ["MD", "DS"]:
            sentence = f"$PNORWD,{dir_type},120720,093150,1,0.05,0.02,2,45.0,90.0"
            pnorwd = PNORWD.from_nmea(sentence)
    ...
    def test_all_spectrum_basis_types(self):
        ...
        for basis in [0, 1, 3]:
            sentence = f"$PNORWD,MD,120720,093150,{basis},0.05,0.02,2,45.0,90.0"
            pnorwd = PNORWD.from_nmea(sentence)
            assert pnorwd.spectrum_basis == basis

    def test_invalid_direction_type(self):
        """Test that invalid direction types are rejected."""
        sentence = "$PNORWD,XX,120720,093150,1,0.05,0.02,2,45.0,90.0*38"
    ...
    def test_invalid_spectrum_basis(self):
        ...
        sentence = "$PNORWD,MD,120720,093150,2,0.05,0.02,2,45.0,90.0*32"
    ...
    def test_value_count_mismatch(self):
        ...
        sentence = "$PNORWD,MD,120720,093150,1,0.05,0.02,5,45.0,90.0,135.0*33"
        with pytest.raises(ValueError, match="Value count mismatch"):
            PNORWD.from_nmea(sentence)

    def test_to_dict(self):
        """Test conversion to dictionary."""
        sentence = "$PNORWD,DS,120720,093150,3,0.03,0.01,4,30.5,45.2,60.8,75.1*2E"
        pnorwd = PNORWD.from_nmea(sentence)
        data = pnorwd.to_dict()
    ...
    def test_large_value_array(self):
        ...
        sentence = f"$PNORWD,MD,120720,093150,1,0.01,0.01,999,{values}"
        pnorwd = PNORWD.from_nmea(sentence)
