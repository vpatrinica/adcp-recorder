...
from adcp_recorder.cli.main import cli, configure, list_ports, start
...
def mock_config(tmp_path):
    ...
    with patch('adcp_recorder.cli.main.RecorderConfig') as MockConfig:
        ...
        MockConfig.load.return_value = instance
...
def test_configure_update(runner, mock_config):
    ...
    mock_config.load.return_value.update.assert_called_with(serial_port='/dev/ttyS1', baudrate=19200)
    assert "Configuration updated" in result.output

def test_start(runner, mock_config):
    with patch('adcp_recorder.cli.main.AdcpRecorder') as MockRecorder:
        result = runner.invoke(cli, ['start'])
        assert result.exit_code == 0
        assert "Starting recorder" in result.output
        
        MockRecorder.assert_called_once()
        MockRecorder.return_value.run_blocking.assert_called_once()
