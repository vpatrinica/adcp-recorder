...
def check_timezone():
    ...
    if aware_local.tzinfo:
        utc_converted = aware_local.astimezone(UTC)
    ...
    res = conn.execute(
        "SELECT * FROM test WHERE ts >= ?", [utc_now_naive - timedelta(seconds=1)]
    ).fetchall()
    ...
    try:
        res = conn.execute(
            "SELECT * FROM test WHERE ts >= ?", [utc_aware - timedelta(seconds=1)]
        ).fetchall()
